...
<!-- GENERATED by scripts/gen_adapters.py from skills/review-spec/SKILL.md.
Do not edit: run the generator instead. The canonical workflow is
reference/review-spec.md; this file only routes an agent to it. -->
...
Follow the canonical procedure in `.cadence/reference/review-spec.md` exactly.

Read `[paths].specs` and `[paths].principles` from the user's config: `python3 .cadence/tools/cadence_config.py --json`.

...
<!-- GENERATED by scripts/gen_adapters.py from skills/spec-driven-development/SKILL.md.
Do not edit: run the generator instead. The canonical workflow is
reference/spec-driven-development.md; this file only routes an agent to it. -->
Follow the canonical procedure in `.cadence/reference/spec-driven-development.md` exactly.
...
Pause for a human when the task is materially ambiguous, changes architecture, changes a data contract, or has wide blast radius. Use a Shape A brief (`.cadence/reference/human-brief.md`).

For a substantial spec, escalate to `.cadence/reference/spec-pipeline.md`: product intent reviewed and merged as its own PR *before* any engineering design begins.

...
<!-- GENERATED by scripts/gen_adapters.py from skills/git-pr-workflow/SKILL.md.
Do not edit: run the generator instead. The canonical workflow is
reference/git-pr-workflow.md; this file only routes an agent to it. -->
Follow the canonical procedure in `.cadence/reference/git-pr-workflow.md` exactly.
...
Read the review state with `python3 .cadence/tools/review_state.py --pr <n>` — **never the check row**, which renders `pass` for a skipped, rate-limited, paused and stale review alike.

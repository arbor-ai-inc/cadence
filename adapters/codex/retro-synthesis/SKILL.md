...
<!-- GENERATED by scripts/gen_adapters.py from skills/retro-synthesis/SKILL.md.
Do not edit: run the generator instead. The canonical workflow is
reference/retro-synthesis.md; this file only routes an agent to it. -->
Follow the canonical procedure in `.cadence/reference/retro-synthesis.md` exactly.

Read `[paths].retros` and `[retro].synthesis_threshold` from `cadence.toml` — `python3 .cadence/tools/cadence_config.py --json` prints both. Never assume the defaults.
